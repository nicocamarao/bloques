'use strict';
importScripts('vendor/jsfeat.min.js','cv-core.js?v=amor-3');
let db;
fetch('targets/compiled.json?v=amor-3').then(r=>{if(!r.ok)throw Error('No se pudieron cargar los envases.');return r.json();}).then(data=>{db=data;db.targets.forEach(t=>t.desc=Int32Array.from(t.desc));postMessage({type:'ready'});}).catch(e=>postMessage({type:'error',message:e.message}));

...
for(const id of ['rose','thin','dreams','longlove','fruit','strong','xxl','dreams-local'])for(const kind of ['front','catalog','gray','rotated','label']){
 ...
 const start=performance.now(),r=CV.detect(rgba,420,430,targets);console.log(id,kind,r?.id,r?.inliers,r?.score,Math.round(performance.now()-start));if(r?.id!==(id==='dreams-local'?'dreams':id))failures++;
}

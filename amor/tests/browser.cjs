...
(async()=>{
 ...
 const detections=await page.evaluate(async()=>{
  ...
  for(const id of ['rose','thin','dreams','longlove','fruit','strong','xxl']){
   ...
   const canvas=document.createElement('canvas');canvas.width=420;canvas.height=430;const context=canvas.getContext('2d');context.fillStyle='white';context.fillRect(0,0,420,430);context.drawImage(image,60,20,300,300*image.naturalHeight/image.naturalWidth);
 ...
 assert.equal(await page.evaluate(()=>gameState.mask),127);
 ...
 assert.equal(await page.locator('#collectionCount').textContent(),'7/7');
 await page.click('.site-menu a[href="#guia-amor"]');
 await page.waitForFunction(()=>{const frame=document.querySelector('#guia-amor iframe');return frame.getBoundingClientRect().top<innerHeight});
 const guide=page.frameLocator('#guia-amor iframe');
 await guide.locator('h1').waitFor({timeout:20000});
 assert.match(await guide.locator('h1').textContent(),/Diferentes formas/);
 await page.screenshot({path:'/tmp/amor-guide.png'});
 // Existing five-creature rewards survive migration and collecting new varieties.
 await page.evaluate(()=>{let old={v:1,mask:31,reward:gameState.reward};document.cookie=AmorGame.cookieName+'='+encodeURIComponent(JSON.stringify(old))+'; Path=/';});
 await page.reload();await page.waitForFunction(()=>!document.getElementById('start').disabled);
 assert.equal(await page.evaluate(()=>gameState.reward.image),reward);
 assert.equal(await page.locator('#collectionCount').textContent(),'5/7');

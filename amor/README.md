...
The local Tesseract.js 5.1.1 fallback accepts distinctive fragments such as ULTRA, CORRUG, FUERTE and XXL at word confidence >=30. Shared words AMOR and EXTRA alone do not select a creature, and conflicting varieties are rejected. The first detection shows the creature; repeated results from the same OCR frame do not count as a second capture confirmation. Results expire after 1.8 seconds and are scoped to the active camera session. OCR runs separately from visual recognition, at most one job at a time, with a 550ms minimum interval. The bundled English LSTM model reads the Latin characters without sending camera frames to a server. The initial OCR download is roughly 7 MB (one of two CPU-compatible cores plus model).

Visual matching now accepts eight geometric inliers instead of ten and lower appearance correlation for ordinary label templates. The higher checks for visually similar Strong/XXL and the fallback correlation gate remain. Real browser OCR tests cover all seven aliases, shared-word rejection and stale sessions; physical-phone accuracy still depends on readable letters, focus and lighting.

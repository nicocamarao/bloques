...
(function(root){
'use strict';
const NAME='amor_hunt_v1',IDS=['rose','thin','dreams','longlove','fruit','strong','xxl'];
const scope=()=>location.pathname.replace(/[^/]*$/,'')||'/';
function read(){let raw=document.cookie.split(';').map(s=>s.trim()).find(s=>s.startsWith(NAME+'='));if(!raw)return {v:2,mask:0};let s;try{s=JSON.parse(decodeURIComponent(raw.slice(NAME.length+1)))}catch{throw Error('No se pudo leer la cookie de esta colección. No se generó otro código.')}if(![1,2].includes(s.v)||!Number.isInteger(s.mask)||s.mask<0||s.mask>(s.v===1?31:127))throw Error('La cookie de la colección no es válida.');if(s.v===1){s.legacyReward=!!s.reward&&s.mask===31;s.v=2;}if(s.reward&&((s.mask!==127&&!(s.legacyReward&&(s.mask&31)===31))||!/^PREMIO:AMOR:[0-9a-f-]{36}$/.test(s.reward.payload)||!/^data:image\/gif;base64,[A-Za-z0-9+/]+=*$/.test(s.reward.image)))throw Error('El código guardado no es válido. No se generó otro.');return s}
...
function ensureReward(s){if(s.mask!==127||s.reward)return s;const payload='PREMIO:AMOR:'+uniqueId();let qr=qrcode(0,'M');qr.addData(payload,'Byte');qr.make();s.reward={payload,image:qr.createDataURL(1,4)};return s}
...
async function restoreReward(){return locked(()=>{let s=read();if(s.mask===127&&!s.reward)return write(ensureReward(s));return s})}

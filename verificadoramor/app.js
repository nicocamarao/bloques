...
function count(){try{const n=Object.keys(AmorVerifier.read()).length;$('count').textContent=n+' QR '+(n===1?'registrado':'registrados')+' en este navegador';}catch{$('count').textContent='No se puede leer el historial local.';}}
...
async function verify(payload){
 ...
 try{
  const record=await AmorVerifier.verify(payload);
  ...
  else result('invalid','QR no válido','No contiene un premio Amor con el formato esperado. No se registró.');
...
window.addEventListener('storage',e=>{if(e.key===AmorVerifier.key||e.key===null)count();});

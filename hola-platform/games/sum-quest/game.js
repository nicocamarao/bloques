(() => {
  ...
  function advanceStageIfNeeded() {
    ...
    if (stage.gateOpen && player.x + player.w > stage.gateX + GATE_WIDTH + 8) {
      ...
      if (currentStage >= world.length) {
        ...
        const score = Math.max(0, 8000 - Math.floor((performance.now() - startTime) / 8));
        window.recordGameScore?.("mundo-numberblocks", score, { label: "Mundo Numberblocks" });
        return;
      }
  ...
  function drawHUD(stage) {
    ...
    if (gameState === "won") {
      ...
      ctx.textAlign = "center";
      ctx.fillText("¡Mundo Numberblocks completado!", w / 2, 96);

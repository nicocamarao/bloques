...
function ensureReward(s){if(s.mask!==15||s.reward)return s;const payload='PREMIO:KINGJR:'+uniqueId();let qr=qrcode(0,'M');qr.addData(payload,'Byte');qr.make();s.reward={payload,image:qr.createDataURL(1,4)};return s}
...
async function restoreReward(){return locked(()=>{let s=read();if(s.mask===15&&!s.reward)return write(ensureReward(s));return s})}
...
root.KingGame={read,write,capture,restoreReward,randomMonster,count,ids:IDS,cookieName:NAME};
})(globalThis);

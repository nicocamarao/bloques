...
for(let yy=0;yy<20;yy++)for(let xx=0;xx<20;xx++){
 ...
 reference.push(sample(t.thumb,24,24,(xx+.5)*24/20,(yy+.5)*24/20));
}
...
if(similarity<(t.partial?.65:.58))return null;
return {id:t.id,quad,inliers:goodFrom.length,score:goodFrom.length/unique.length,variant:t.variant}}
...
function ncc(im,targets){let scale=Math.min(1,320/Math.max(im.cols,im.rows)),w=Math.round(im.cols*scale),h=Math.round(im.rows*scale),small=new J.matrix_t(w,h,J.U8_t|J.C1_t);J.imgproc.resample(im,small,w,h);let best=null;
for(let t of targets){if(!t.thumb)continue;let tmp={cols:24,rows:24,data:t.thumb};t.coarse=t.coarse||normal(patch(tmp,0,0,23,23,8));t.fine=t.fine||normal(patch(tmp,0,0,23,23,20));let candidates=[];
for(let bw=18;bw<Math.min(w*.98,320);bw*=1.19){let bh=bw*t.h/t.w;if(bh<8||bh>h*.98)continue;let step=Math.max(2,Math.round(Math.min(bw,bh)/10));for(let y=0;y<h-bh-1;y+=step)for(let x=0;x<w-bw-1;x+=step){let c=corr(small,x,y,bw,bh,8,t.coarse);if(Math.abs(c)>.60){candidates.push({x,y,bw,bh,c:Math.abs(c),step});if(candidates.length>60){candidates.sort((a,b)=>b.c-a.c);candidates.length=18}}}}
candidates.sort((a,b)=>b.c-a.c);for(let q of candidates.slice(0,16))for(let ds of [.95,1,1.05])for(let dy of [-q.step/2,0,q.step/2])for(let dx of [-q.step/2,0,q.step/2]){let bw=q.bw*ds,bh=q.bh*ds,x=q.x+dx,y=q.y+dy;if(x<0||y<0||x+bw>=w||y+bh>=h)continue;let score=Math.abs(corr(small,x,y,bw,bh,20,t.fine));if(score>(t.partial?.84:.80)&&(!best||score>best.score))best={id:t.id,variant:t.variant,score,inliers:0,quad:[{x:x/scale,y:y/scale},{x:(x+bw)/scale,y:y/scale},{x:(x+bw)/scale,y:(y+bh)/scale},{x:x/scale,y:(y+bh)/scale}]}}
}return best}
function detect(rgba,w,h,targets){
 let im=gray(rgba,w,h),f=features(im,750),best=null;f.image=im;
 for(let t of targets){let r=match(f,t,w,h);if(r&&(!best||r.inliers>best.inliers))best=r}
 return best||ncc(im,targets.filter(t=>!t.variant.includes('rotation')))||ncc(im,targets.filter(t=>t.variant.includes('rotation')));
}

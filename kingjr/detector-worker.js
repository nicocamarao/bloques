'use strict';
importScripts('vendor/jsfeat.min.js?v=3','cv-core.js?v=kingjr-3');
let db;
fetch('targets/compiled.json?v=kingjr-3').then(r=>{if(!r.ok)throw Error('No se pudieron cargar los logos.');return r.json()}).then(data=>{db=data;db.targets.forEach(t=>t.desc=Int32Array.from(t.desc));postMessage({type:'ready'})}).catch(e=>postMessage({type:'error',message:e.message}));
